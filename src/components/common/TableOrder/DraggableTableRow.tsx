...
export const DraggableTableRow = ({ row, header, urlEdit, urlDetail, handleOpen, onDelete, dragable, orderIds, onClose }: Props) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <TableRow
      ...
      tabIndex={-1}
      key={row.id}
      sx={{ cursor: 'pointer', background: orderIds !== undefined ? (orderIds.find((id: any) => id == row.id) ? '#FFFFCC' : '') : '' }}
      ref={setNodeRef}
      style={style}
    >
      {
